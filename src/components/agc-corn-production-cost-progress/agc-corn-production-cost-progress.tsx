...
@Component({
    tag: 'agc-corn-production-cost-progress'
})
export class AgcCornProductionCostProgress {
    @Prop() socket:string = ""
    @State() currentStep = -1
    progress: HTMLDivElement;
        ...
        return (
            <div ref={c => this.progress = c as HTMLDivElement} class="wizard__progress">
                                ...
                                <span class={`step${this.currentStep > 2 ? ' finish': ''}${this.currentStep === 2 ? ' active' : ''}`}></span>              
                                <span class={`step${this.currentStep > 3 ? ' finish': ''}${this.currentStep === 3 ? ' active' : ''}`}></span>              
                            </div>
        );
    }
