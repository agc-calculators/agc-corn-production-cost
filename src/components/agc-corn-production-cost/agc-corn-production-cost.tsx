...
@Component({
  tag: "agc-corn-production-cost"
})
export class AgcCornProductionCost {
  @Prop() socket: string = "";
  @Prop() tract: string = "";
  @Prop() mode: "full" | "step" = "step";
  @Prop() units: any = { land: "acre", yield: "bu/acre", corn: "bu" };
  @State() currentStep = 0;
  @State() cache = {};
    ...
    return (
      <div>
        <form
          ...
          data-wizard="agc-corn-production-cost"
        ...
        >
          ...
          <section data-wizard-section="0">
            <div class="agc-wizard__field">
              <label data-i18n="fields.corn-market-price">
                Estimated Corn Market Price
              </label>
              <input
                name="cornMarketPrice"
                type="number"
                required
                min="1"
                step=".01"
              ...
              <p
                ...
                data-i18n="validation.corn-market-price.required"
                data-validates="cornMarketPrice"
              >
                Please enter a price.
              </p>
              <p data-i18n={`hints.corn-market-price.${this.units["corn"]}`}>
                ⮤ Enter the expected market price for corn per bushel.
              </p>
            </div>
          ...
          <section data-wizard-section="3">
            <div class="agc-wizard__field">
              <label data-i18n={`fields.variable-costs.${this.units['land']}`}>Variable Costs per Acre</label>
              <input
                name="variableCosts"
                type="number"
                required
  ...
  showResults() {
    let cornMarketPrice = parseFloat((this.form.querySelector(
      '[name="cornMarketPrice"'
    ) as HTMLInputElement).value);
    let expectedYield = parseInt((this.form.querySelector(
      '[name="expectedYield"'
    ) as HTMLInputElement).value);
    let plantedAcres = parseInt((this.form.querySelector(
      '[name="plantedAcres"'
    ) as HTMLInputElement).value);
    let variableCosts = parseFloat((this.form.querySelector(
      '[name="variableCosts"'
    ) as HTMLInputElement).value);
