...
class AgcCornProductionCost {
    constructor() {
        this.socket = "";
        this.tract = "";
        this.mode = "step";
        this.units = { land: "acre", yield: "bu/acre", corn: "bu" };
        this.currentStep = 0;
        this.cache = {};
    ...
    render() {
        return (h("div", null,
            h("form", { onSubmit: e => e.preventDefault(), ref: c => (this.form = c), "data-wizard": "agc-corn-production-cost", "data-wizard-mode": this.mode, class: "agc-wizard" },
                h("slot", null),
                h("section", { "data-wizard-section": "0" },
                    h("div", { class: "agc-wizard__field" },
                        h("label", { "data-i18n": "fields.corn-market-price" }, "Estimated Corn Market Price"),
                        h("input", { name: "cornMarketPrice", type: "number", required: true, min: "1", step: ".01" }),
                        h("p", { class: "agc-wizard__validation-message", "data-i18n": "validation.corn-market-price.required", "data-validates": "cornMarketPrice" }, "Please enter a price."),
                        h("p", { "data-i18n": `hints.corn-market-price.${this.units["corn"]}` }, "\u2BA4 Enter the expected market price for corn per bushel.")),
    ...
    validateForm() {
        ...
        if (this.currentStep === 0 || this.mode === "full") {
            if (!validate(this.form, "cornMarketPrice")) {
                valid = false;
            }
        }
    ...
    showResults() {
        let cornMarketPrice = parseFloat(this.form.querySelector('[name="cornMarketPrice"').value);
        let expectedYield = parseInt(this.form.querySelector('[name="expectedYield"').value);
        let plantedAcres = parseInt(this.form.querySelector('[name="plantedAcres"').value);
        let variableCosts = parseFloat(this.form.querySelector('[name="variableCosts"').value);
        let totalIncomePerAcre = round(expectedYield * cornMarketPrice, 2);
    ...
    static get is() { return "agc-corn-production-cost"; }
